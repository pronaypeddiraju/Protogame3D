...
// Vertex Shader
//--------------------------------------------------------------------------------------
VertexToFragment_t VertexFunction(VertexShaderInput_t vertexFragInput)
{

    VertexToFragment_t vertexToFragment = (VertexToFragment_t)0;
    
    // The output of a vertex shader is in clip-space, which is a 4D vector
    // so we need to convert out input to a 4D vector.
	vertexToFragment.ndc_position = SOME_POSITIONS[vertexFragInput.VertexIndex];  
	vertexToFragment.position = float4( SOME_POSITIONS[vertexFragInput.VertexIndex], 1.0f );
...
// target.
float4 FragmentFunction(VertexToFragment_t vertexFragInput) : SV_Target0 // semeantic of what I'm returning
{
   // We just output a solid colour - in this case, white. 
   return float4( vertexFragInput.ndc_position, 1.0f );
}